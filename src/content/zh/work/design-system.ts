...
export const designSystem: CaseStudyContent = {
  ...
  keyDecisions: [
    ...
    },
    {
      ...
      paragraphs: [
        "UI／UX 團隊透過 Daily Stand-up 同步正在製作的頁面、元件變化與 Token 需求，提早辨識同類需求是否已在其他產品出現；每週的 Design QA 檢查跨產品影響，避免不同團隊為相似情境各自建立一套規格。提出調整時，設計師需說明使用情境、產品目的、既有元件無法涵蓋的原因與可能共用範圍；前端評估實作方式、可控參數、相容性及對既有頁面的影響；PM 則確認需求優先順序、例外條件與驗收範圍。",
  ...
  execution: {
    ...
    paragraphs: [
      ...
      "在這段期間，我負責規劃導入與治理方式，主持 Daily Stand-up 與 Design QA，協助團隊提早發現跨產品的相似需求，並確認元件的共用範圍。當產品需求與既有規則發生衝突時，我會判斷應調整共用元件、增加可控的 Variant／Props，或保留具有明確用途的產品特規。這套做法讓元件庫能隨真實產品情境逐步成熟，也避免為了涵蓋每一項差異而持續增加變數，最後變得難以理解、實作與維護。",
    ],
    supportingMetrics: [
