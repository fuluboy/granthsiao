...
export const designSystem: CaseStudyContent = {
  ...
  keyDecisions: [
    ...
    },
    {
      ...
      heading: "把元件決策放進日常開發流程",
      paragraphs: [
        "新需求進入 Sprint 後，設計、前端與 PM 會一起確認使用情境、共用範圍、實作方式與驗收條件；Daily Stand-up 提早暴露跨產品的相似需求，Weekly Design QA 則處理共用範圍與例外。",
        "當需求和既有規則衝突時，我協助判斷要延伸共用元件、增加可控的 Variant／Props，或保留有明確用途的產品特規。決策會記錄適用情境與限制，之後若其他產品出現相似需求，再重新評估是否適合整併。",
      ],
      infoCard: {
        label: "治理節奏",
        rows: [
  ...
  execution: {
    ...
    paragraphs: [
      "我以 Chief of Experience 身分帶領 3 位 UI／UX，三個產品團隊則共由 2 位前端支援。若先花數月做完一套完整元件庫，不只會拖慢產品，也可能在缺乏真實情境時做出後來用不到的規格。因此我們把 Design System 建置直接放進原本兩週一次的 Sprint，優先處理當期真的需要、同時具備共用價值的元件。",
      "我的工作是讓設計、前端與 PM 在同一套判斷方式下工作，確認哪些需求值得共用、哪些差異應被保留，並讓 Figma Library、前端元件與正式產品持續對得上。這樣元件庫能跟著真實產品逐步成熟，不需要先停下三個團隊再做一次全面重構。",
    ],
    supportingMetrics: [
    ...
    roleCard: {
      ...
      items: [
        ...
        "參與 Daily Stand-up 並主持 Design QA",
        "確認跨產品共用範圍與元件優先順序",
