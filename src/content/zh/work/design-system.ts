...
export const designSystem: CaseStudyContent = {
  ...
  keyDecisions: [
    ...
    },
    {
      ...
      paragraphs: [
        "UI／UX 團隊每天同步正在進行的頁面與元件需求，每週再檢查哪些變動可能影響其他產品。新增或調整 Token 時，設計師先說明使用情境與共用可能；前端確認實作與既有元件影響，PM 則確認產品情境與優先順序。",
        "完成對齊後，設計元件與前端元件在同一個 Sprint 更新。即使是單一產品的特殊元件，也會保留在 Library 並標明用途，讓例外有原因、有邊界，也能被後續維護。",
      ],
    },
  ],
  execution: {
    ...
    paragraphs: [
      "我以 Chief of Experience 身分帶領 3 位 UI／UX，三個產品團隊共由 2 位前端支援，其中一位需要同時服務兩個團隊。我們沒有額外人力先花幾個月完成一套完整元件庫，因此把建置工作放進原本兩週一次的 Sprint。",
      "前期由設計團隊盤點畫面與 Token；進入產品開發後，優先挑選當期真正會使用的按鈕、表單與 Modal，由設計與前端在同一輪完成規格、實作與驗證。我負責安排順序、確認共用範圍，並在產品需求與共用規則衝突時，判斷應調整元件、增加可控變數，或保留產品特規。",
    ],
    supportingMetrics: [
